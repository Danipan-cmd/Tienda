...
tecnologias = [
    ...
    Tecnologias("Sony WH-1000XM4", 1300000, "Sony", "Audífonos"),
...
def agregar_producto():
        ...
        if (categoria_seleccionada == "Verduras" and p.verdura == producto_seleccionado) or \
           ...
           (categoria_seleccionada == "Panaderias" and p.panaderia == producto_seleccionado) or \
            ...
            if categoria_seleccionada in ["Verduras", "Carnes"]:
                ...
                precio_total = precio_por_kilo * cantidad_seleccionada
                kilos = cantidad_seleccionada
            else:
                precio_por_unidad = p.precio
                precio_total = precio_por_unidad * cantidad_seleccionada
    ...
    for i, (prod, cant, precio, kls, desc) in enumerate(productos_agregados):
        if prod == producto_seleccionado:
            if kilos > 0:
                # Recalcular el precio total para la cantidad total
                precio_total_nuevo = (precio_por_kilo * (kilos + cantidad_seleccionada))
                descuento_aplicado_nuevo = precio_total_nuevo * descuento
                precio_total_nuevo -= descuento_aplicado_nuevo
                productos_agregados[i] = (producto_seleccionado, cant + cantidad_seleccionada, precio_total_nuevo, kilos + cantidad_seleccionada, descuento_aplicado_nuevo)
            else:
                # Recalcular el precio total para la cantidad total
                precio_total_nuevo = (precio_por_unidad * (cant + cantidad_seleccionada))
                descuento_aplicado_nuevo = precio_total_nuevo * descuento
                precio_total_nuevo -= descuento_aplicado_nuevo
                productos_agregados[i] = (producto_seleccionado, cant + cantidad_seleccionada, precio_total_nuevo, kls, descuento_aplicado_nuevo)
            break
    else:
...
def generar_factura():
    ...
    for producto, cantidad, precio, kilos, descuento in productos_agregados:
        if kilos > 0:
            factura += f"{producto} (x{kilos} kg): ${precio:.2f}"
            if descuento > 0:
                factura += f" (Descuento: ${descuento:.2f})"
            factura += "\n"
        else:
            factura += f"{producto} (x{cantidad} unidades): ${precio:.2f}"
            if descuento > 0:
                factura += f" (Descuento: ${descuento:.2f})"
            factura += "\n"
        total += precio
